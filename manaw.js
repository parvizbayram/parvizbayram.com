(() => {
  ...
  function initScrollToTop() {
    const button = document.querySelector("[data-manaw-scroll-top]");
    if (!button) {
      return;
    }
  ...
  function blockMediaContextMenus() {
    const mediaBlocks = document.querySelectorAll(".manaw-media");
    ...
    const videoBlocks = document.querySelectorAll(".manaw-media.work-video");
    videoBlocks.forEach((block) => {
      if (block.querySelector(".manaw-media-shield")) {
        return;
      }

      const shield = document.createElement("div");
      shield.className = "manaw-media-shield";
  ...
  window.addEventListener("load", syncManawScale, { once: true });
  window.addEventListener("resize", syncManawScale, { passive: true });
})();
