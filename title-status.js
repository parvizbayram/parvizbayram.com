(() => {
  ...
  function faviconHref(color) {
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><circle cx="32" cy="32" r="32" fill="${color}"/></svg>`;
  ...
  function syncFavicon(isInactive) {
    ...
    icon.href = faviconHref(isInactive ? INACTIVE_FAVICON : ACTIVE_FAVICON);
  }

  function getActiveTitle() {
    const pathname = window.location.pathname.replace(/\/$/, "") || "/";
    return ROUTE_TITLES.get(pathname) || DEFAULT_ACTIVE_TITLE;
  }
  ...
  function syncTitle() {
    ...
    document.title = isInactive ? INACTIVE_TITLE : getActiveTitle();
